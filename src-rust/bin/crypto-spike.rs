...
fn main() {
    ...
    if args.len() < 3 {
        ...
        eprintln!();
        eprintln!("This is the Phase 0.1 crypto spike to validate whether Rust can decrypt Go-generated data.");
        std::process::exit(1);
    }
    ...
    match command.as_str() {
        ...
        "decrypt-provider" => {
            if args.len() < 4 {
                eprintln!("Error: decrypt-provider requires <path-to-database.json> and <master-key-hex>");
                std::process::exit(1);
            }
...
fn test_decrypt_master_key(path: &str, passphrase: &str) {
    ...
    match decrypt_with_scrypt(&content, passphrase) {
        ...
        Err(e) => {
            ...
            eprintln!("Error: {}", e);
            eprintln!();
            eprintln!("This indicates a compatibility issue between Go and Rust age implementations.");
...
fn decrypt_with_scrypt(ciphertext: &str, passphrase: &str) -> Result<Vec<u8>, Box<dyn std::error::Error>> {
    ...
    {
        let mut reader = decryptor.decrypt(&mut std::iter::once(&scrypt_identity as &dyn age::Identity))?;
...
fn decrypt_provider_ciphers(database_path: &str, master_key_hex: &str) {
    ...
    println!("Master key (hex): {}...", &master_key_hex[..min(16, master_key_hex.len())]);
    println!();
